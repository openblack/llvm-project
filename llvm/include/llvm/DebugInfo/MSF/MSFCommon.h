...
static const char LesserMagic[] = {'M',  'i',  'c',    'r', 'o', 's',  'o',  'f',
                             't',  ' ',  'C',    '/', 'C', '+',  '+',  ' ',
                             'p',  'r',  'o',    'g', 'r', 'a',  'm',  ' ',
                             'd',  'a',  't',    'a', 'b', 'a',  's',  'e',
                             ' ',  '2',  '.',    '0', '0', '\r', '\n', '\x1a',
...
struct MSFLesserLayout {
  MSFLesserLayout() = default;

  uint32_t mainFpmBlock() const {
    assert(LB->FreeBlockMapBlock == 1 || LB->FreeBlockMapBlock == 2);
    return LB->FreeBlockMapBlock;
  }
  ...
  const LesserBlock *LB = nullptr;
  BitVector FreePageMap;
  ArrayRef<support::ulittle16_t> DirectoryBlocks;
  ArrayRef<std::array<support::ulittle32_t, 2>> StreamSizes; // Interlaced Size and Reserved
  std::vector<ArrayRef<support::ulittle16_t>> StreamMap;
};
...
/// the file.
MSFStreamLayout getFpmStreamLayout(const MSFLesserLayout &Msf,
...
inline uint32_t getFpmIntervalLength(const MSFLesserLayout &L) {
  return L.LB->BlockSize;
}
...
inline uint32_t getNumFpmIntervals(const MSFLesserLayout &L,
                                   bool IncludeUnusedFpmData = false,
                                   bool AltFpm = false) {
  return getNumFpmIntervals(L.LB->BlockSize, L.LB->NumBlocks,
...
Error validateLesserBlock(const LesserBlock &LB);
