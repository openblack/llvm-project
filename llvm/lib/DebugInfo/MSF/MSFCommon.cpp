...
Error llvm::msf::validateLesserBlock(const LesserBlock &LB) {
  // Check the magic bytes.
  if (std::memcmp(LB.MagicBytes, LesserMagic, sizeof(LesserMagic)) != 0)
    return make_error<MSFError>(msf_error_code::invalid_format,
                                "MSF magic header doesn't match");

  if (!isValidBlockSize(LB.BlockSize))
    return make_error<MSFError>(msf_error_code::invalid_format,
                                "Unsupported block size.");

  // We don't support directories whose sizes aren't a multiple of four bytes.
  if (LB.NumDirectoryBytes % sizeof(support::ulittle32_t) != 0)
    return make_error<MSFError>(msf_error_code::invalid_format,
  ...
  uint64_t NumDirectoryBlocks =
      bytesToBlocks(LB.NumDirectoryBytes, LB.BlockSize);

  // The directory, as we understand it, is a block which consists of a list of
  // block numbers.  It is unclear what would happen if the number of blocks
  // couldn't fit on a single block.
  if (NumDirectoryBlocks > LB.BlockSize / sizeof(support::ulittle16_t))
    return make_error<MSFError>(msf_error_code::invalid_format,
                                "Too many directory blocks.");

  if (LB.FreeBlockMapBlock != 1 && LB.FreeBlockMapBlock != 2)
...
MSFStreamLayout llvm::msf::getFpmStreamLayout(const MSFLesserLayout &Msf,
  ...
  if (IncludeUnusedFpmData)
    FL.Length = NumFpmIntervals * Msf.LB->BlockSize;
  else
    FL.Length = divideCeil(Msf.LB->NumBlocks, 8);
